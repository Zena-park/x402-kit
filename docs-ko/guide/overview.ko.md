...
결제 한 건은 평범한 HTTP 왕복입니다: 구매자가 유료 엔드포인트를 호출하면 판매자가
결제 조건을 담아 `402 Payment Required`로 응답하고, 구매자는 그 조건에 서명해 —
트랜잭션도 가스도 없이 — 다시 호출하며, 판매자는 facilitator로 서명을 검증하고
온체인 정산까지 마친 뒤 응답을 내줍니다.

방향에 주목하세요: 402는 **서버가 클라이언트에게** 보냅니다. 그래서 구매자는
클라이언트(에이전트 스크립트, 브라우저 탭, 크론 잡)이고, 서버로 떠 있어야 하는
